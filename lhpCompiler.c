 ...
 *
 * @author Iqra Haq
 * @param[in] file_name, type, lhp_log 
 * @param[out] FILE (FILE variable that stored the opened file.)
 * @date 01/01/2020
 */
FILE *file_opener(char *file_name, const char *type, FILE *lhp_log)
{

	/// FILE parameter specified that is local to this function only.
    FILE *in_file_ptr;
    
    /// fopen used to open the file based on the file name and file type specified.
 ...
 * 
 * @author Iqra Haq
 * @param[out] char * (char array variable that stores the current data and time.)
 * @date 01/01/2020
 */
 ...
 * 
 * @author Iqra Haq
 * @param[in] lhp_file, linker_file, line.
 * @date 01/01/2020
 */
 ...
 * 
 * @author Iqra Haq
 * @param[in] file_name, command, lhp_log.
 * @date 01/01/2020
 */
...
int main (int argc, char* argv[])
{
    ...
    if(lhp_log == NULL){
        ...
        ///Rewind the files that were analysed or modified so that the program can be rerun without any file analysis or modification issues.
        rewind(lhp_file);
        rewind(linker_file);
